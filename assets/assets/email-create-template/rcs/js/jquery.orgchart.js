...
(function (factory) {
  ...
  OrgChart.prototype = {
    ...
    loopChart: function ($chart, includeNodeData) {
      ...
      console.log(">>>>>>>hararky me bani >>>>>>>>",$node);
      
      var subObj = { 'id': $node[0].id,
      'name' : $node[0].textContent ,
    ...
    createNode: function (data) {
      ...
      var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (data[opts.nodeId] ? ' id="' + data[opts.nodeId] + '"' : '') + (data.parentId ? ' data-parent="' + data.parentId + '"' : '') + '>')
        ...
        {
                   $nodeDiv.append('<button class="btn btn-primary">'+data[opts.nodeTitle]+'</button>')
          .append(typeof opts.nodeContent !== 'undefined' ? '<div class="content">' + (data[opts.nodeContent] || '') + '</div>' : '');
        }
        if(data.title === 'text')
        {
                   $nodeDiv.append('<span class="form-control">'+data[opts.nodeTitle]+'</span>')
          .append(typeof opts.nodeContent !== 'undefined' ? '<div class="content">' + (data[opts.nodeContent] || '') + '</div>' : '');
        }
        if(data.title === 'image')
        {
                   $nodeDiv.append('<div class="img_size"> <img  class="img-thumbnail" src="https://templates.joomla-monster.com/joomla30/jm-news-portal/components/com_djclassifieds/assets/images/default_profile.png"></img> </div>')
          .append(typeof opts.nodeContent !== 'undefined' ? '<div class="content">' + (data[opts.nodeContent] || '') + '</div>' : '');
        }
        if(data.title === 'input')
        {
                   $nodeDiv.append('<div class="form-group"><input class="form-control" value="'+data[opts.nodeTitle]+'"></input></div>')
          .append(typeof opts.nodeContent !== 'undefined' ? '<div class="content">' + (data[opts.nodeContent] || '') + '</div>' : '');
        }
      
        if(data.title === 'campain'){

        $nodeDiv.append('<div class="title tree_h1"><h1> ' + data[opts.nodeTitle] + '<h1></div>')
          .append(typeof opts.nodeContent !== 'undefined' ? '<div class="content">' + (data[opts.nodeContent] || '') + '</div>' : '');
      }
      ...
      console.log('>>>create node ka data >>>>>>>>',data );
